...
export const Shapes = ({
  ...
  const textClasses = `flex items-center py-28 md:py-16 lg:py-12 xl:py-0`;
  return (
    <div className="flex flex-col lg:justify-between xl:flex-row">
      <div className={`${shapeWrapperClasses} ${variants[variant].wrapper}`}>
        <img
          ...
          alt="/"
          height={717}
          className={`${photoClasses} ${variants[variant].photo} `}
        />
        ...
        <div className={`${circleClasses} ${variants[variant].circle}`}>
          ...
          <span className="absolute text-5xl font-bold sm:text-7xl md:text-5xl pt-28 md:pt-0 md:font-normal font-brilon text-calicoPink-400 md:text-calicoGray-400">
            {circleText}
          </span>
        </div>
      ...
      <div className={`${textClasses} ${variants[variant].mobileTextAligned}`}>
        <span className="text-2xl sm:text-[2.5rem] leading-normal text-center font-brilon sm:pt-5 md:pt-0 text-calicoGray-400">
          Starting from R{priceText}
        </span>
      </div>
    </div>
  );
